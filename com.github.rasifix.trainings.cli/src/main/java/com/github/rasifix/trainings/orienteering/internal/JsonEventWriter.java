...
import com.github.rasifix.trainings.orienteering.Split;
...
//			writer.member("ascent", category.getAscent());
	...
	private void writerAthlete(Athlete athlete, JsonOutputHandler writer) {
		...
		writer.member("startTime", athlete.getStartTime().format(TimeFormat.HOUR_MINUTE));
		if (athlete.getTotalTime() != null) {
			writer.member("totalTime", athlete.getTotalTime().format(TimeFormat.HOUR_MINUTE_SECOND));
		} else {
	...
	private void writerSplits(Athlete athlete, JsonOutputHandler writer) {
		...
		for (Split split : athlete.getSplits()) {
			if (split.getTime() != null) {
				writer.value(split.getTime().format(TimeFormat.MINUTE_SECOND));
			} else {
				writer.nullValue();
			}
		}
