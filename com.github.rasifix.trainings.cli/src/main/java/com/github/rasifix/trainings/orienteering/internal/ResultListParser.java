...
class ResultListParser {
	...
	protected static void parseRunners(List<ResultListEntry> entries, BufferedReader reader) throws IOException {
		...
		while (line != null) {
			...
			try {
				entry.totalTime = Time.valueOf(split[5].substring(0, split[5].length() - "</b>".length()), TimeFormat.HOUR_MINUTE_SECOND);
	...
	private static Time parseTime(String field) {
		...
		return Time.valueOf(field, TimeFormat.MINUTE_SECOND);
	}
